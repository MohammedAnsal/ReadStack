...
export const FeedPage = () => {
  ...
  const loadPage = useCallback(
    async (pageToLoad: number, replace = false) => {
      ...
      try {
        ...
        const response = await articleService.getFeed(pageToLoad, PAGE_SIZE);
        if (response.success && response.articles) {
          ...
          // backend returns hasMore; fall back to length check
          const more =
  ...
  // Optional: when switching tag, just re-render filtered list; no reset needed.
  // If you want to reset to first page on tag change, uncomment below:
  // useEffect(() => {
  //   setHasMore(true);
  //   loadPage(1, true);
  // }, [activeTag, loadPage]);
  ...
  return (
    <main className="min-h-screen bg-linear-to-br from-gray-50 via-white to-gray-50 text-gray-900">
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            >
              ...
              {!hasMore && (
                <div className="text-xs text-gray-400">No more articles</div>
              )}
            </div>
          </div>
        )}
      ...
      <style>{`
        ...
        .animate-slide-down-delay { animation: slide-down 0.6s ease-out 0.1s both; }
        .animate-slide-down-delay-2 { animation: slide-down 0.6s ease-out 0.2s both; }
